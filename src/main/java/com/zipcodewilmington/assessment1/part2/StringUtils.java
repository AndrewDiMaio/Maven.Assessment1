...
public class StringUtils {
    ...
    String[] test = sentence.split(" ",sentence.length());
    ...
    public static String reverseFirstWordThenCamelCase(String sentence) {
        ...
        StringBuilder reverse = new StringBuilder();
        reverse.append(test[0]).reverse();
        String nowString = reverse.toString();
        StringBuilder cap = new StringBuilder();
        for (int i = 0; i<reverse.length(); i++){
            char temp = nowString.toCharArray()[i];
            if (i == 0){

                cap.append(Character.toUpperCase(temp));
            }else {
                cap.append(temp);
            }
        }
